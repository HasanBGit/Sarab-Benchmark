"""
Mode 3 (Sarab-icc, Incorrect Context) evaluation via OpenRouter.

Protocol verified directly against Liu et al. 2025 (CVPR), the paper
Sarab-icc mirrors -- read via info/Liu_PhD_A_ChatGPT-Prompted_Visual_
...
script -- modes/mode3_icc/sarab_test_set_icc.json now carries
...
TEST_SET = DATA_ROOT / "modes" / "mode3_icc" / "sarab_test_set_icc.json"
...
def mk_result(task, model, raw, parsed, error_type, detail, attempts, latency, reasoning_off,
    ...
    return {
        ...
        "attempts": attempts, "latency_s": latency, "model_name": model, "provider": provider,
        "modality": "text_only" if blind else "vision",
...
def run_one_task(session, api_key, model, task, min_gap, max_retries, state, last_call,
    ...
    for attempt in range(1, max_retries + 1):
        ...
        last_call[0] = t0
        try:
            resp = call_model(session, api_key, model, task["image_path"], prompt_text, max_tokens, reasoning_off,
                               timeout=60, provider=provider, blind=blind)
...
def cmd_run(args):
    ...
    out_path = Path(args.out) if args.out else RESULTS_DIR / f"results_{MODE_LABEL}_{model_slug}{blind_suffix}.csv"
